...
function withStatus(Component) {
  return function (props) {
    if (props.views >= 1000) {
      return (
        <Popular>
          <Component views={props.views} title={props.title} url={props.url} />
        </Popular>
      );
...
const StatusArticle = withStatus(Article);
const StatusVideo = withStatus(Video);
...
function List(props) {
  return props.list.map((item) => {
    switch (item.type) {
      case "video":
        return <StatusVideo {...item} />;

      case "article":
        return <StatusArticle {...item} />;
    }
  });
}
